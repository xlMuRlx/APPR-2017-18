...
Poleg vseh zgoraj opisanih analiz, sem se dodatno odločil še, da bom dodal spustni seznam, pri katerem lahko uporabnik izbere eno izmed dosedanjih udeleženk svetovnih prvenstev, program pa mu izpiše njeno najboljšo uvrstitev ter prvenstvo, na katerem jo je nazadnje doseglo. Za zadnji del, torej da program izpiše le prvenstvo, na katerem je izbrana država nazadnje dosegla svojo najbolšo uvrstitev, sem se odločil zaradi lažje sestave seznama , po katerem se podatki berejo ter posledično lažje zasnove programa.

```{r shiny, echo = FALSE}
shinyAppDir("shiny", options=list(width="100%", height=220))
```  
